...
DOCUMENTATION = '''
---
module: cisco_ucs_vnic_ether_if
short_description: configures vnic ether if on a cisco ucs server profile's ls server
version_added: 0.9.0.0
description:
   -  configures vnic ether if on a cisco ucs server profile's ls server
options:
    state:
        description:
...
def setup_vnic_ether_if(server, module):
    ...
    exists, mo = vnic_ether_if_exists(handle=server, **args_mo)

    if ansible["state"] == "present":
        if module.check_mode or exists:
            return not exists
        vnic_ether_if_create(handle=server, **args_mo)
    else:
        if module.check_mode or not exists:
            return exists
        vnic_ether_if_delete(server, mo.name, args_mo['org_dn'])
...
def setup(server, module):
    ...
    try:
        result["changed"] = setup_vnic_ether_if(server, module)
