...
DOCUMENTATION = '''
---
module: cisco_ucs_vnic_fc
short_description: configures vnic fc on a cisco ucs server profile's ls server
...
def _argument_mo():
    return dict(
                ...
                cdn_prop_in_sync=dict(type='str'),
...
def setup_vnic_fc(server, module):
    ...
    exists, mo = vnic_fc_exists(handle=server, **args_mo)

    if ansible["state"] == "present":
        if module.check_mode or exists:
            return not exists
        vnic_fc_create(handle=server, **args_mo)
    else:
        if module.check_mode or not exists:
            return exists
        vnic_fc_delete(server, mo.name, args_mo['org_dn'])
...
def setup(server, module):
    ...
    try:
        result["changed"] = setup_vnic_fc(server, module)
